...
static int sf_ctl_device_init_gpio_pins(struct sf_ctl_device *ctl_dev)
{
    ...
    int_gpio = of_get_named_gpio(ctl_dev->pdev->dev.of_node, "int-gpio", 0);
	ctl_dev->irq_num=gpio_to_irq(int_gpio);
	
	xprintk(KERN_INFO, "irq number is %d.\n", ctl_dev->irq_num);
	//if (gpio_is_valid(ctl_dev->irq_num)) 
    
    {
        int i = 0;

        for (i = 0; i < SF_PIN_STATE_MAX; ++i) {
			xprintk(KERN_ERR, "YYY  '%s' pinctrl_state.\n",sf_pinctrl_state_names[i]);
