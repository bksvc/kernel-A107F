...
static void forward_rule_add_old(int i, char *tunnel_src, char *tunnel_dst)
{
	...
	if (ismobike) {
		memcpy(tmp->tunnel_ip_s, tunnel_src, strlen(tunnel_src));
		memcpy(tmp->tunnel_ip_d, tunnel_dst, strlen(tunnel_dst));
		tmp->cnt = 1;
	} else
		++tmp->cnt;
...
int forward_rule_add_new(char *src, __u8 pref_s, char *dst, __u8 pref_d, char *tunnel_src, char *tunnel_dst, char *ims_ifname, __u16 family)
{
	...
	for(i=0; i < RECORD_COUNT; i++) {
		...
		if(tmp->used == 0) {
			...
			memcpy(tmp->tunnel_ip_s, tunnel_src, strlen(tunnel_src));
			memcpy(tmp->tunnel_ip_d, tunnel_dst, strlen(tunnel_dst));
			memcpy(tmp->ims_ifname, ims_ifname, strlen(ims_ifname));
			tmp->pref_s = pref_s;
			tmp->pref_d = pref_d;
			tmp->family = family;
			tmp->used = 1;
